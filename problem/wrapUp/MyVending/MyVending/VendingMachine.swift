...
class VendingMachine {
    ...
    func changeToDataDic() {
        for drinkName in stockDic.keys {
            dataDic[drinkName] = stockDic[drinkName]?.count
        }
    }
    ...
    func saveData() {
        ...
        self.changeToDataDic()
        let tempDic = self.getStockDic()
        print("save : \(tempDic)")
        userDefaults.set(tempDic, forKey: "data")
    }
    
    func bringData() {
        let savedDic = UserDefaults.standard.dictionary(forKey: "data") as? [String : Int]
        print(savedDic)
        if let temp = savedDic {
            dataDic = temp
        }
    }
}
